...
export default async function VehiclePageDe({ params }: { params: Promise<Params> }) {
  const { type } = await params
  const vehicle = vehicles.find((v) => v.type === type)
  if (!vehicle) notFound()

  const pageUrl = `${siteUrl}/de/fleet/${type}`
    ...
    <>
      <JsonLd
        data={{
          ...
          itemListElement: [
            ...
            { '@type': 'ListItem', position: 2, name: 'Fuhrpark', item: `${siteUrl}/de/fleet` },
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 md:grid-cols-2 md:items-center">
          <div>
            ...
            <p className="mt-4 text-sm font-semibold text-brand-gold">
              {vehicle.passengers} Fahrgäste · {vehicle.luggage}
            </p>
            <Link
              href={`/de/booking?to=${encodeURIComponent(vehicle.name)}`}
      ...
      <section className="mx-auto max-w-4xl px-4 py-16 sm:px-6">
        <div className="grid gap-10 sm:grid-cols-2">
          {vehicle.idealFor && vehicle.idealFor.length > 0 && (
            <div>
              <h2 className="font-display text-xl text-brand-ink">Ideal für</h2>
          ...
          {vehicle.features && vehicle.features.length > 0 && (
            <div>
              <h2 className="font-display text-xl text-brand-ink">Enthaltene Ausstattung</h2>
              <ul className="mt-3 space-y-2 text-sm text-brand-ink-2">
      ...
      <section className="border-t border-brand-line bg-white">
        <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
          <h2 className="font-display text-2xl text-brand-ink">Weitere Fahrzeuge</h2>
