...
import { blogPosts, type BlogBlock } from '@/lib/content/de/blog'
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: { en: `/blog/${slug}`, de: canonical, 'x-default': `/blog/${slug}` },
    },
    openGraph: {
      type: 'article',
      siteName,
        ...
        <section className="border-b border-brand-line bg-brand-cream">
          <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
            ...
            <p className="mt-4 text-xs text-brand-ink-2/60">
              Von {siteName} Team · Veröffentlicht am{' '}
              <time dateTime={post.publishedAt}>{formatDate(post.publishedAt)}</time>
              {post.updatedAt && (
                <>
                  {' '}
                  · Aktualisiert am <time dateTime={post.updatedAt}>{formatDate(post.updatedAt)}</time>
                </>
              )}{' '}
              · {post.readingTime}
            </p>
          ...
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div className="max-w-[700px] space-y-6">
              ...
              <div className="rounded-sm border border-brand-line bg-brand-cream p-6 text-center sm:p-8">
                <p className="font-display text-lg text-brand-ink">Bereit für diesen Transfer?</p>
                <p className="mt-1.5 text-sm text-brand-ink-2/80">
                  Senden Sie Ihre Reisedaten und wir bestätigen Verfügbarkeit und einen Festpreis per E-Mail.
                </p>
                <Link
                  href="/de/booking"
            ...
            <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
              <HeroQuoteCard locale="de" title="Festpreis anfragen" />

              {tocEntries.length > 0 && (
                <div className="rounded-sm border border-brand-line bg-white p-5">
                  <p className="font-display text-sm text-brand-ink">In diesem Artikel</p>
              ...
              {post.relatedPages && post.relatedPages.length > 0 && (
                <div className="rounded-sm border border-brand-line bg-white p-5">
                  <p className="font-display text-sm text-brand-ink">Weiterführende Seiten</p>
