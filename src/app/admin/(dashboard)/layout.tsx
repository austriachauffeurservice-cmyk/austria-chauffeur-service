  ...
  const navContent = (
    <>
      <div style={{ padding: '20px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 32, height: 32, flexShrink: 0, background: `linear-gradient(135deg,${c.gold},${c.goldLight})`, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14, color: c.bg }}>A</div>
        <div>
          <div style={{ fontWeight: 600, fontSize: 14, color: c.text, lineHeight: 1.2 }}>Austria Chauffeur</div>
          <div style={{ fontSize: 11, color: c.textFaint }}>Admin Portal</div>
        </div>
      </div>
      ...
      <nav style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 2, flex: 1 }}>
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <FontAwesomeIcon icon={item.icon} style={{ width: 15 }} />
              {item.label}
            </Link>
          )
        })}
      ...
      <div style={{ padding: 12, borderTop: `1px solid ${c.border}`, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        >
          <FontAwesomeIcon icon={faRightFromBracket} style={{ width: 15 }} /> Sign Out
        </button>
      </div>
    </>
  )
      ...
      <div className="admin-shell">
        ...
        <div className="admin-main">
          <div className="admin-mobile-bar">
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: c.text }}>Austria Chauffeur · Admin</span>
          </div>
          {children}
        </div>
      </div>
